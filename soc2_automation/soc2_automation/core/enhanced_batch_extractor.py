...
def main():
    ...
    reports_dir = "/home/levi/sandbox/soc2_automation/soc2_automation/data/input/reports/"
    ...
    # Extract data from all reports with timestamped folder
    from datetime import datetime
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_dir = f"../data/processed/runs/{timestamp}"
    os.makedirs(run_dir, exist_ok=True)
    
    output_file = f"{run_dir}/company_organized_reports.json"
    extracted_reports = extractor.extract_from_reports(report_files, output_file)
    
    # Update latest symlink
    latest_link = "../data/processed/latest"
    if os.path.exists(latest_link) or os.path.islink(latest_link):
        os.unlink(latest_link)
    os.symlink(f"runs/{timestamp}", latest_link)
    ...
    if extracted_reports:
        ...
        for company, company_reports in by_company.items():
            if len(company_reports) > 0:
                company_filename = f"{run_dir}/{company.replace(' ', '_').replace('/', '_')}_reports.json"
